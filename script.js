 ...
 * Note: This provides the base click/keyboard functionality.
 * The ScrollDrivenStepper module handles the scroll-lock behavior.
 */
function initStepper() {
  ...
  // Note: ScrollDrivenStepper also handles clicks for its pause mechanism
  stepperNavItems.forEach(item => {
  ...
  // Keyboard support (Arrow keys for navigation within stepper)
  ...
  function startAutoPlay() {
    ...
    // Don't auto-play if scroll-driven stepper is locked
    if (typeof ScrollDrivenStepper !== 'undefined' && ScrollDrivenStepper.isLocked && ScrollDrivenStepper.isLocked()) {
      return;
    }
    
    stepperAutoPlayInterval = setInterval(() => {
      // Skip if scroll-driven stepper is locked
      if (typeof ScrollDrivenStepper !== 'undefined' && ScrollDrivenStepper.isLocked && ScrollDrivenStepper.isLocked()) {
        return;
      }
...
// ==========================================================================
// SCROLL-DRIVEN STEPPER CONTROLLER (METHOD SECTION)
// ==========================================================================

/**
 * Scroll-Driven Stepper Controller
 * Transforms the Method section into a scroll-hijacked stepper experience
 * when the section is fully visible in the viewport.
 */
const ScrollDrivenStepper = (function() {
  // Configuration
  const CONFIG = {
    deltaThreshold: 220,        // Trackpad accumulator threshold
    cooldownMs: 1800,           // Cooldown between step changes
    clickPauseMs: 1400,         // Pause after click before scroll resumes
    unlockNudge: 30,            // Small nudge on unlock (px)
    visibilityTolerance: 20,    // Tolerance for "fully visible" check (px)
    mobileBreakpoint: 768       // Don't lock on mobile
  };
  ...
  /**
   * Check if the method section is fully (or nearly fully) visible
   */
  function isSectionFullyVisible() {
    ...
    // Section must be fully in viewport with tolerance
    return rect.top >= -CONFIG.visibilityTolerance && 
           rect.bottom <= viewportHeight + CONFIG.visibilityTolerance;
  }
  ...
  /**
   * Lock the scroll on the method section
   */
  function lockScroll() {
    ...
    // Add wheel handler specifically to method section
    if (!boundWheelHandler) {
      boundWheelHandler = handleWheel;
    }
    methodSection.addEventListener('wheel', boundWheelHandler, { passive: false });
    
    // Also listen on window to catch all wheel events while locked
  ...
  function unlockScroll(direction) {
    ...
    methodSection.removeEventListener('wheel', boundWheelHandler);
    window.removeEventListener('wheel', handleWindowWheel);
    
    // Small nudge in the direction of scroll to provide continuity
    if (direction && CONFIG.unlockNudge > 0) {
      const nudgeAmount = direction === 'down' ? CONFIG.unlockNudge : -CONFIG.unlockNudge;
      window.scrollBy({ top: nudgeAmount, behavior: 'auto' });
    }
  }

  /**
   * Handle wheel events on the method section
   */
  function handleWheel(e) {
    if (!isLocked) return;
    
    // During click pause, ignore wheel
    ...
    // Cooldown check
    const now = Date.now();
    if (now - lastStepChangeTime < CONFIG.cooldownMs) {
      e.preventDefault();
      return;
    }
    ...
    if (Math.abs(deltaAccumulator) >= CONFIG.deltaThreshold) {
      ...
      if (direction === 'down') {
        if (currentStep < totalSteps) {
          ...
          // At last step, unlock and continue scroll
          unlockScroll('down');
        }
      } else {
        if (currentStep > 1) {
          ...
          // At first step, unlock and continue scroll up
          unlockScroll('up');
        }
      }
    } else {
  ...
  /**
   * Handle wheel events on window (to catch events outside method section while locked)
   */
  function handleWindowWheel(e) {
    if (!isLocked) return;
    
    // If wheel happens outside method section while locked, delegate
    if (!methodSection.contains(e.target)) {
      handleWheel(e);
    }
  }

  /**
   * Handle scroll to check if we should lock
   */
  function handleScroll() {
    if (prefersReducedMotion() || isMobile()) return;
    if (!methodSection) return;
    
    // If already locked, don't interfere
    if (isLocked) return;
  ...
  /**
   * Handle click on stepper nav items
   */
  function handleNavClick(e) {
    const navItem = e.target.closest('.stepper-nav-item');
    if (!navItem) return;
    
    const step = parseInt(navItem.dataset.step);
    if (!isNaN(step)) {
  ...
  function handleKeydown(e) {
    ...
    // Only handle if focus is within stepper nav
    const activeElement = document.activeElement;
    if (!activeElement || !activeElement.closest('.stepper-nav')) return;

    if (e.key === 'Enter' || e.key === ' ') {
      const step = parseInt(activeElement.dataset.step);
      if (!isNaN(step)) {
        e.preventDefault();
        activateStep(step, true);
      }
  ...
  function initARIA() {
    ...
    stepperNavItems.forEach((item, index) => {
      const step = parseInt(item.dataset.step);
      const card = document.querySelector(`.stepper-card[data-step="${step}"]`);
      ...
      if (card) {
        const cardId = `stepper-card-${step}`;
        card.id = cardId;
        item.setAttribute('aria-controls', cardId);
        card.setAttribute('role', 'tabpanel');
        card.setAttribute('aria-labelledby', `stepper-nav-${step}`);
        item.id = `stepper-nav-${step}`;
    ...
    // Set up nav as tablist
    const stepperNav = document.querySelector('.stepper-nav');
    if (stepperNav) {
      stepperNav.setAttribute('role', 'tablist');
      stepperNav.setAttribute('aria-label', 'Étapes de la méthode');
    }
  }
  ...
  function init() {
    ...
    if (prefersReducedMotion()) {
      console.log('ScrollDrivenStepper: Disabled (prefers-reduced-motion)');
      return;
    }
    ...
    if (stepperNavItems.length === 0 || stepperCards.length === 0) {
      console.log('ScrollDrivenStepper: Stepper elements not found');
      return;
    }
    ...
    const activeNav = methodSection.querySelector('.stepper-nav-item.active');
    if (activeNav) {
      currentStep = parseInt(activeNav.dataset.step) || 1;
    }
    ...
    // Click handler for nav items (uses event delegation)
    const stepperNav = methodSection.querySelector('.stepper-nav');
    if (stepperNav) {
      stepperNav.addEventListener('click', handleNavClick);
    }
  ...
  function destroy() {
    ...
    if (methodSection) {
      const stepperNav = methodSection.querySelector('.stepper-nav');
      if (stepperNav) {
        stepperNav.removeEventListener('click', handleNavClick);
      }
      if (boundWheelHandler) {
        methodSection.removeEventListener('wheel', boundWheelHandler);
      }
...
function cleanup() {
  ...
  // Cleanup scroll-driven stepper
  ScrollDrivenStepper.destroy();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Scroll-driven stepper controller (desktop only)
  ScrollDrivenStepper.init();
