...
function initStepper() {
  ...
  // Scroll-driven activation (cards advance as you scroll through the section)
  const methodSection = document.getElementById('method');
  const stepsCount = stepperNavItems.length || stepperCards.length;
  let lastManualInteraction = 0;
  let inView = false;
  let sectionTop = 0;
  let sectionHeight = 0;
  let scrollActiveStep = null;
  ...
  function updateStepFromScroll() {
    if (!methodSection || !inView) return;

    const now = performance.now();
    // Avoid fighting the user right after a click/key action
    if (now - lastManualInteraction < 1200) return;

    // Focus line inside viewport (tweak if you want earlier/later switching)
    const focusY = window.scrollY + window.innerHeight * 0.45;

    const start = sectionTop;
    const end = sectionTop + sectionHeight - window.innerHeight * 0.55;
    const denom = Math.max(1, end - start);

    const progress = Math.min(1, Math.max(0, (focusY - start) / denom));
    const stepIndex = Math.min(stepsCount - 1, Math.floor(progress * stepsCount));
    const stepNumber = stepIndex + 1;

    if (scrollActiveStep !== stepNumber) {
      scrollActiveStep = stepNumber;
  ...
  if (methodSection) {
    ...
    visObserver.observe(methodSection);
...
function initROICalculator() {
  ...
  const roiStatSubtitle = document.getElementById('roi-stat-subtitle');
  ...
  // Scratch content elements
  const scratchDaysValue = document.getElementById('scratch-days-value');
  const scratchFallbackValue = document.getElementById('scratch-fallback-value');
  ...
  if (!form || !roiResultValue || !scratchCard) {
   ...
   * @param {number} days
   */
  function updateScratchContent(days) {
    if (scratchDaysValue) {
      scratchDaysValue.textContent = `+${days}`;
    }
    if (scratchFallbackValue) {
      scratchFallbackValue.textContent = `+${days}`;
    }
  }
  ...
  function handleROICalculation(e) {
    ...
    // Update ROI display
    roiResultValue.textContent = formatCurrency(annualGain);
    roiResultValue.dataset.calculated = 'true';
    roiResultValue.classList.add('is-calculated');
    
    // Update subtitle
    if (roiStatSubtitle) {
      roiStatSubtitle.textContent = `Soit +${daysRecovered} jours/an récupérés`;
      roiStatSubtitle.classList.add('calculated');
    }
    
    // Update breakdown panel
    updateBreakdownPanel(result);
    
    // Update scratch content with days
    updateScratchContent(daysRecovered);
    
    // Set flag and reveal scratch card
    roiCalculated = true;
    revealScratchCard();
    
    // Add pulse animation to stat
    roiResultValue.classList.add('is-active');
    setTimeout(() => {
      roiResultValue.classList.remove('is-active');
    }, 2000);
    
    console.log(`ROI calculated: ${formatCurrency(annualGain)}, ${daysRecovered} days recovered`);
  }
  ...
  function handleInputChange() {
    if (roiCalculated) {
      ...
      // Reset ROI display
      roiResultValue.textContent = '—';
      roiResultValue.dataset.calculated = 'false';
      roiResultValue.classList.remove('is-calculated');
      
      if (roiStatSubtitle) {
        roiStatSubtitle.textContent = 'Recalculez après modification';
        roiStatSubtitle.classList.remove('calculated');
      }
