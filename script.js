 ...
 * Version 5.3 - Scroll-driven Method Stepper + Unified ROI Cards + Integrated Scratch Reveal
...
function initStepper() {
  ...
  const stepperCards = Array.from(document.querySelectorAll('.stepper-card'));
  const stepperNavItems = Array.from(document.querySelectorAll('.stepper-nav-item'));

  if (!stepperNav || stepperCards.length === 0 || stepperNavItems.length === 0) {
    return;
  }

  // Reduced motion: show everything, no fancy interactions
  if (prefersReducedMotion()) {
  ...
  // ------------------------------------------------------------------------
  // ARIA + IDs wiring (done in JS to avoid touching HTML)
  // ------------------------------------------------------------------------
  stepperNav.setAttribute('role', 'tablist');

  stepperNavItems.forEach((item, idx) => {
    const step = parseInt(item.dataset.step || (idx + 1), 10);
    const tabId = `method-step-tab-${step}`;
    const panelId = `method-step-panel-${step}`;

    item.id = item.id || tabId;
    item.setAttribute('role', 'tab');
    item.setAttribute('aria-controls', panelId);
    item.setAttribute('tabindex', item.classList.contains('active') ? '0' : '-1');
    // Keep aria-pressed for backwards compatibility + add aria-selected for tabs
    item.setAttribute('aria-selected', item.classList.contains('active') ? 'true' : 'false');

    const panel = stepperCards.find(c => parseInt(c.dataset.step || '0', 10) === step);
    if (panel) {
  ...
  /**
   * Returns active step number.
   * @returns {number}
   */
  function getActiveStep() {
    const currentActive = document.querySelector('.stepper-nav-item.active');
    return parseInt(currentActive?.dataset.step || '1', 10);
  }

  /**
   * Sets a pause window after a click so scroll doesn't immediately "take over".
   * @param {number} ms
   */
  function setClickPause(ms = 1400) {
    clickPauseUntil = Date.now() + ms;
  }
   ...
   * @param {Object} [opts]
   * @param {"click"|"keyboard"|"scroll"|"api"} [opts.source]
   */
  function activateStep(stepNumber, opts = {}) {
    const source = opts.source || 'api';

    // Clamp
    const maxStep = Math.max(...stepperNavItems.map(i => parseInt(i.dataset.step || '1', 10)));
    const next = Math.min(Math.max(stepNumber, 1), maxStep);

    // Update nav items
    stepperNavItems.forEach(item => {
      const isActive = parseInt(item.dataset.step || '0', 10) === next;
    ...
    // Update cards (use hidden to keep SR output clean)
    stepperCards.forEach(card => {
      const isActive = parseInt(card.dataset.step || '0', 10) === next;
    ...
    // Notify scroll-stepper (if present)
    try {
      if (window.__methodScrollStepper && typeof window.__methodScrollStepper.onStepChange === 'function') {
        window.__methodScrollStepper.onStepChange(next, source);
      }
    } catch (_) {}

    // Auto-play reset if enabled
  ...
  stepperNavItems.forEach(item => {
    item.addEventListener('click', () => {
      const step = parseInt(item.dataset.step || '1', 10);
      activateStep(step, { source: 'click' });
      setClickPause(1500);
    });
  });

  // ------------------------------------------------------------------------
  // Keyboard support (ArrowLeft/Right to navigate tabs)
  // ------------------------------------------------------------------------
  stepperNav.addEventListener('keydown', (e) => {
    ...
    let nextStep;
    const maxStep = Math.max(...stepperNavItems.map(i => parseInt(i.dataset.step |

/**
 * Scroll-driven Stepper (Method section)
 * - Locks the page scroll ONLY when #method is fully visible.
 * - While locked, wheel/trackpad scroll advances/reverses exactly one step (with threshold + cooldown).
 * - Unlocks at last step (scroll down) or first step (scroll up).
 * - Keeps click navigation and adds a short pause after click.
 */
function initScrollDrivenMethodStepper() {
  const api = window.__methodStepper;
  const methodSection = api?.elements?.methodSection || document.getElementById('method');

  if (!api || !methodSection) return;

  // Respect reduced motion: no lock, no scroll hijack
  if (prefersReducedMotion()) return;

  // Desktop-only: keep mobile/touch natural (accordion already handles UX)
  const mqDesktop = window.matchMedia('(min-width: 768px)');
  const mqCoarse = window.matchMedia('(pointer: coarse)');
  if (!mqDesktop.matches || mqCoarse.matches) return;
  ...
  // Keep a handle for step change notifications (optional)
  window.__methodScrollStepper = window.__methodScrollStepper || {};
  window.__methodScrollStepper.onStepChange = () => {};

  const normalizeDeltaY = (evt) => {
    let dy = evt.deltaY;
    if (evt.deltaMode === 1) dy *= 16; // lines to px
    if (evt.deltaMode === 2) dy *= window.innerHeight; // pages to px
    return dy;
  };

  const isFullyVisible = () => {
    const rect = methodSection.getBoundingClientRect();
    return rect.top >= -settings.fullVisibleEpsilon &&
           rect.bottom <= window.innerHeight + settings.fullVisibleEpsilon;
  };

  const shouldIgnoreEvent = (evt) => {
    // Don't hijack when user interacts with inputs / selects etc
    const t = evt?.target;
    if (!t) return false;
  ...
  const addLockedListeners = () => {
    window.addEventListener('wheel', onWheel, { passive: false });
    window.addEventListener('keydown', onKeyDown, true);
  };

  const removeLockedListeners = () => {
  ...
  const lock = () => {
    ...
    methodSection.classList.add('is-locked');
    // Help prevent scroll chaining on some browsers
    methodSection.style.overscrollBehavior = 'contain';

    addLockedListeners();
  };

  const unlock = (direction = 0, nudgePx = settings.unlockNudgePx) => {
    if (!locked) return;
    locked = false;
    accumulator = 0;

    methodSection.classList.remove('is-locked');
    methodSection.style.overscrollBehavior = '';

    removeLockedListeners();

    // Nudge to preserve scroll "continuity" after the unlock intent
    if (direction !== 0) {
      requestAnimationFrame(() => {
        window.scrollBy({ top: direction * nudgePx, left: 0, behavior: 'auto' });
      });
    }
  };

  const stepCount = api.stepsCount || 5;

  const tryStep = (direction) => {
    const now = Date.now();

    // Pause after click
    if (now < (api.clickPauseUntil || 0)) return;

    // Cooldown
    if (now - lastStepAt < settings.cooldownMs) return;

    const current = api.getActiveStep();
    const next = current + direction;

    // Unlock rules
    if (current === stepCount && direction > 0) {
      unlock(1);
      return;
    }
    ...
    api.activateStep(next, { source: 'scroll' });
    lastStepAt = now;
  };
  ...
  function onWheel(evt) {
    ...
    if (window.scrollY !== lockScrollY) {
      window.scrollTo({ top: lockScrollY, left: 0, behavior: 'auto' });
    }

    evt.preventDefault();

    const dy = normalizeDeltaY(evt);
    accumulator += dy;

    // If user changes direction, reset accumulator to keep behaviour stable
    if ((accumulator > 0 && dy < 0) || (accumulator < 0 && dy > 0)) {
      accumulator = dy;
    }

    if (Math.abs(accumulator) < settings.thresholdPx) return;
  ...
  function onKeyDown(evt) {
    ...
    // Allow Esc to bail out (accessibility safety hatch)
    if (evt.key === 'Escape') {
      unlock(0, 0);
      return;
    }
    ...
    if (![...keysDown, ...keysUp].includes(evt.key)) return;

    // Don't steal keystrokes from inputs
    if (shouldIgnoreEvent({ target: document.activeElement })) return;

    evt.preventDefault();

    const direction = keysDown.includes(evt.key) ? 1 : -1;
    const nudge = (evt.key === 'PageDown' || evt.key === 'PageUp' || evt.key === ' ' || evt.key === 'Spacebar')
      ? Math.round(window.innerHeight * 0.8)
      : settings.unlockNudgePx;

    // Unlock if at boundaries with same continuity expectations
    const current = api.getActiveStep();
    if (current === stepCount && direction > 0) {
      unlock(1, nudge);
      return;
  ...
  // ------------------------------------------------------------------------
  // Freeze trigger: only when the section is fully visible.
  // We use IntersectionObserver to avoid constantly checking off-screen.
  // ------------------------------------------------------------------------
  const scheduleCheck = () => {
    ...
    rafId = requestAnimationFrame(() => {
      rafId = null;
      if (!locked && isFullyVisible()) lock();
    });
  };

  const onScrollOrResize = () => scheduleCheck();

  const io = new IntersectionObserver((entries) => {
    const entry = entries[0];
    if (!entry) return;

    // Start watching when section is in/near viewport
    if (entry.isIntersecting) {
      ...
      // Stop watching when out of view (unless locked)
      if (watching && !locked) {
        watching = false;
    ...
    root: null,
    threshold: 0,
    rootMargin: '200px 0px 200px 0px' // small prefetch window
  });

  io.observe(methodSection);

  // Safety: if user clicks a nav link while locked, unlock first
  document.addEventListener('click', (e) => {
    if (!locked) return;
    const target = e.target;
    if (!(target instanceof Element)) return;
    const link = target.closest('a[href^="#"], a[href^="http"], button');
    if (!link) return;

    // Clicking inside the stepper should not unlock
    if (methodSection.contains(link)) return;

    unlock(0, 0);
  }, true);

  console.log('Scroll-driven Method Stepper initialized');
}

| '1', 10)));

    if (e.key === 'ArrowRight') {
      nextStep = currentStep < maxStep ? currentStep + 1 : 1;
    } else {
      nextStep = currentStep > 1 ? currentStep - 1 : maxStep;
    }

    activateStep(nextStep, { source: 'keyboard' });
  ...
  function startAutoPlay() {
    ...
    stepperAutoPlayInterval = setInterval(() => {
      const currentStep = getActiveStep();
      const maxStep = Math.max(...stepperNavItems.map(i => parseInt(i.dataset.step || '1', 10)));
      const nextStep = currentStep < maxStep ? currentStep + 1 : 1;
      activateStep(nextStep, { source: 'api' });
  ...
  if (CONFIG.stepper.autoPlayEnabled && methodSection) {
  ...
  // ------------------------------------------------------------------------
  // Expose a tiny API for the scroll-driven controller
  // ------------------------------------------------------------------------
  window.__methodStepper = {
    activateStep,
    getActiveStep,
    stepsCount: stepperNavItems.length,
    setClickPause,
    get clickPauseUntil() { return clickPauseUntil; },
    get elements() {
      return { methodSection, stepperNav, stepperNavItems, stepperCards };
    }
  };

  // Ensure an initial SR-friendly state: hide inactive cards
  stepperCards.forEach(card => {
    if (!card.classList.contains('active')) card.setAttribute('hidden', 'true');
  });
